...
function addtomusic(argument){
	...
	for (const track of track_list) {
		if(track.track_id==id) {
			if(track.MyMusic) {
				argument.classList.add("Active_AddTrack_btn");
			} else {
				argument.classList.remove("Active_AddTrack_btn");
			}
			break
		}
	}
...
function get_track_html(title,author,image,id,time) {
	return '<div class="track" onclick="StartTrack(this)" data='+id+'>\
		...
		<div class="actions">\
			<svg class="add-icon" width="46" height="46" onclick="addtomusic(this)" data='+id+' viewBox="0 0 46 46" fill="none" xmlns="http://www.w3.org/2000/svg">\
