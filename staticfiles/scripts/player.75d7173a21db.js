...
function playpause(argument){
	...
	if(myAudio.paused) {
		if(track_loaded){
			...
			myAudio.load();
			myAudio.src="";
			//download_flag.style.display="block";
	...
	return $.ajax({
		...
		success: function(response){
			...
			for (const index in response) {
				...
				MusicList_box.innerHTML+=get_track_html(title=data["name"],author=data["author"],image=data["image"],id=data["id"]);
...
function get_track_html(title,author,image,id) {
...
var pos={x:0,y:0};
function set_point(percent){
	var angle=percent*360;
	var d=describeArc(50,50,48,0,angle);
	if(d==false) return false;
	if(angle>=360) {
		full_arc.setAttribute("stroke","#000000");
...
var updateTimer=setInterval(UpdateRange,20);//20
*/
